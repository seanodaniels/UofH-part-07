...
test('verify that new entries can be created via POST', async () => {
  ...
  expect(titles).toContain('This is a test for a new blog list entry.')
...
test('verify that new entries fail if token not provided', async () => {
  ...
  await api.post('/api/blogs').send(newEntry).expect(401)
...
test('verifies that if like property is missing it is set to 0', async () => {
  ...
  const newEntry = modifiedDb.find(
    (entry) =>
      entry.title ===
      'This is a test for an absent like property a8923iIdkzqlp88.'
  )
...
test('verify delete works properly', async () => {
  ...
  await api.delete(`/api/blogs/${condemnedEntry.id}`).expect(204)
...
test('verify update via put', async () => {
  ...
  await api.put(`/api/blogs/${changedEntry.id}`).send(changedEntry).expect(200)
