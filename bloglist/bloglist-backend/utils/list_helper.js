...
const favoriteBlog = (blogs) => {
  const reducer = (accum, item) => {
    return item.likes > accum.likes ? (accum = item) : accum
  }
...
const mostBlogs = (blogs) => {
  ...
  for (blog in blogs) {
    ...
    if (totalsArray[0]) {
      const blogFind = totalsArray.find(
        (element) => currentAuthor === element.author
      )
  ...
  const reducer = (accum, item) => {
    return item.blogs > accum.blogs ? (accum = item) : accum
  }
